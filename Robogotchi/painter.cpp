...
vector<vector<double>> Painter::clearPIX(const int& We, const int& Hi, const int& maxVal) const {
...
vector<vector<QColor>> Painter::clearCOL(const int& We, const int& Hi, const QColor& col) const {
...
QImage Painter::createZ(const vector<vector<point>>& input, const light& _light, const vector<QColor>& colors, const vector<vector<bool>>& shadows) const {
...
void Painter::draw(const vector<vector<point>>& input, const light& _light, const vector<QColor>& colors) const {
    const Transform trans;
    vector<double> paramSh = vector<double>{-_light.param[0], -_light.param[1], -_light.param[2]};
    QImage result;
    if (_light.shadow) {
        const vector<vector<bool>> shadows = calculateShadow(trans.RotatePoints(input, paramSh), _light);
...
QColor Painter::createColor(vector<point> part, QColor input, light _light) const {
    MathVector n = MathVector(part[1], part[2]) * MathVector(part[1], part[0]);
...
vector<vector<bool>> Painter::calculateShadow(const vector<vector<point>>& input, const light& _light) const {
    vector<vector<double>> pix = clearPIX(W, H, MAXZ);
    int ymax = 0, ymin = 0, ysc = 0, e1 = 0, e = 0;
    for (auto part: input) {
...
QImage Painter::drawLight(QImage& img, const light& _light) const {
